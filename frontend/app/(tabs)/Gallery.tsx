...
const Gallery: React.FC = () => {
  ...
  const fetchImages = useCallback(async () => {
    setIsLoading(true);
    try {
      const response = await fetch('http://192.168.31.52:8000/api/images/');
  ...
  const handleImagePress = (imageUrl: string) => {
    setFullScreenImage(`http://192.168.31.52:8000${imageUrl}`);
  };
  ...
  const renderImageItem = ({ item }: { item: ImageType }) => (
    ...
    >
      <Image
        source={{ uri: `http://192.168.31.52:8000${item}` }}
        style={styles.image}
      />
